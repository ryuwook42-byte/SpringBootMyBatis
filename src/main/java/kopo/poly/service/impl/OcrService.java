...
public class OcrService implements IOcrService {
    ...
    static {
        // mac(Homebrew)에 설치된 libtesseract.dylib을 JNA가 찾을 수 있도록 경로 지정
        // tess4j의 TessAPI가 로딩되기 전에(static 블럭이므로 클래스 로딩 시점에) 설정되어야 함
        String jnaLibPath = "/opt/homebrew/lib";
        if (new File(jnaLibPath).exists()) {
            String existing = System.getProperty("jna.library.path");
            System.setProperty("jna.library.path",
                    existing == null || existing.isBlank() ? jnaLibPath : existing + ":" + jnaLibPath);
        }
    }
